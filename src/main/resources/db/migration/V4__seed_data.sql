...
INSERT INTO brands (name) VALUES
  ('Honda'), ('Toyota'), ('Hyundai'), ('Renault'), ('Fiat'), ('BMW'), ('Mercedes'), ('Volkswagen')
ON CONFLICT (name) DO NOTHING;
...
INSERT INTO providers (name, address, city, district, phone, location)
VALUES
  ('Ostim Honda Usta', '1003. Cad. No:10', 'Ankara', 'Yenimahalle', '+90 312 000 0001', ST_SetSRID(ST_MakePoint(32.7540, 39.9776), 4326)),
  ('İvedik Elektrik', '1473. Sok. No:3',    'Ankara', 'Yenimahalle', '+90 312 000 0002', ST_SetSRID(ST_MakePoint(32.7432, 39.9819), 4326)),
  ('Siteler Kaporta', 'Sanayi Cd. No:25',    'Ankara', 'Altındağ',   '+90 312 000 0003', ST_SetSRID(ST_MakePoint(32.8859, 39.9477), 4326)),
  ('Keçiören Lastik', 'Fatih Cd. No:48',     'Ankara', 'Keçiören',   '+90 312 000 0004', ST_SetSRID(ST_MakePoint(32.8670, 39.9921), 4326)),
  ('Çayyolu Boya',    'Konya Yolu Blv. 8',   'Ankara', 'Çankaya',    '+90 312 000 0005', ST_SetSRID(ST_MakePoint(32.7306, 39.8865), 4326))
...
SELECT p.id, c.id FROM p JOIN c ON (
...
SELECT p.id, b.id FROM (SELECT id, name FROM providers) p
...
INSERT INTO ratings (provider_id, score, comment_text, user_id, anonymous_id, ip_address)
VALUES
  ((SELECT id FROM p WHERE name='Ostim Honda Usta'),  5,  'Çok ilgili ve hızlı', (SELECT id FROM u WHERE display_name='Ali'),   null, '1.2.3.4'),
  ((SELECT id FROM p WHERE name='Ostim Honda Usta'),  3,  'İdare eder',           null,                                       'anon-123', '2.3.4.5'),
  ((SELECT id FROM p WHERE name='İvedik Elektrik'),  -1,  'Teslimat gecikti',      (SELECT id FROM u WHERE display_name='Ayşe'), null, '3.4.5.6'),
  ((SELECT id FROM p WHERE name='Siteler Kaporta'),   4,  'İşçilik temiz',         null,                                       'anon-456', '4.5.6.7'),
  ((SELECT id FROM p WHERE name='Keçiören Lastik'),   2,  'Fiyat/performans iyi',  (SELECT id FROM u WHERE display_name='Mehmet'), null, '5.6.7.8'),
